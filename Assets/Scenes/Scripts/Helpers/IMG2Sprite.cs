...
public static class IMG2Sprite
{
    ...
    public static Sprite LoadNewSprite(byte[] data, float PixelsPerUnit = 100.0f, SpriteMeshType spriteType = SpriteMeshType.Tight)
    {

        // Load a PNG or JPG image from disk to a Texture2D, assign this texture to a new sprite and return its reference

        Texture2D SpriteTexture = LoadTexture(data);
        Sprite NewSprite = Sprite.Create(SpriteTexture, new Rect(0, 0, SpriteTexture.width, SpriteTexture.height), new Vector2(0, 0), PixelsPerUnit, 0, spriteType);
    ...
    public static Texture2D LoadTexture(byte[] data)
    {
        ...
        Tex2D.LoadImage(data);         // Load the imagedata into the texture (size is set automatically)
        return Tex2D;
    }
}
